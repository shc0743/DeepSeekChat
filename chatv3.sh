...
while true; do
  ...
  # 将用户输入添加到对话历史中
  MESSAGES=$(echo "$MESSAGES" | jq --arg role "user" --arg content "$ESCAPED_MESSAGE" '. += [{"role": $role, "content": $content}]')
  ...
  # 将助手的回复添加到对话历史中
  MESSAGES=$(echo "$MESSAGES" | jq --arg role "assistant" --arg content "$ASSISTANT_MESSAGE" '. += [{"role": $role, "content": $content}]')
